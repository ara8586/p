...
if is_banned(data.id_, arg.chat_id) then
   if not lang then
		tdcli.sendMessage(arg.chat_id, arg.msg_id, 0, "⛔️_User_ "..user_name.." *[ "..data.id_.." ]* _is banned_⛔️", 0, "md")
   else
		tdcli.sendMessage(arg.chat_id, arg.msg_id, 0, "⛔️_کاربر_ "..user_name.." *[ "..data.id_.." ]* _از گروه محروم است_⛔️", 0, "md")
end
kick_user(data.id_, arg.chat_id)
end
if is_gbanned(data.id_) then
     if not lang then
		tdcli.sendMessage(arg.chat_id, arg.msg_id, 0, "⛔️_User_ "..user_name.." *[ "..data.id_.." ]* _is globally banned_⛔️", 0, "md")
    else
		tdcli.sendMessage(arg.chat_id, arg.msg_id, 0, "⛔️_کاربر_ "..user_name.." *[ "..data.id_.." ]* _از تمام گروه های ربات محروم است_⛔️", 0, "md")
   end
kick_user(data.id_, arg.chat_id)
     end
	end
...
tdcli.deleteMessagesFromUser(data.chat_id_, data.sender_user_id_, dl_cb, nil)
   if not lang then
  return tdcli.sendMessage(data.chat_id_, "", 0, "⚠️_All_ *messages* _of_ *[ "..data.sender_user_id_.." ]* _has been_ *deleted*⚠️", 0, "md")
      elseif lang then
  return tdcli.sendMessage(data.chat_id_, "", 0, "⚠️*تمام پیام های* *[ "..data.sender_user_id_.." ]* *پاک شد*⚠️", 0, "md")
       end
    end
  end
else
    if lang then
  return tdcli.sendMessage(data.chat_id_, "", 0, "_کاربر یافت نشد_⚠️", 0⚠️, "md")
   else
...
kick_user(matches[2], msg.chat_id_)
 ...
 if matches[1] == "delall" or matches[1] == "حذف همه" and is_mod(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
...
return tdcli.sendMessage(msg.chat_id_, msg.id_, 0, "❕*کاربر "..matches[2].." از محرومیت گروه خارج شد*❕", 0, "md")
 ...
 if matches[1] == "silent" or matches[1] == "سایلنت" and is_mod(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
...
data[tostring(chat)]['is_silent_users'][tostring(matches[2])] = ""
 ...
 if matches[1] == "unsilent" or matches[1] == "رفع سایلنت" and is_mod(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
   ...
   tdcli_function ({
      ...
      username_ = matches[2]
		...
		if matches[1]:lower() == 'clean' or matches[1]:lower() == 'حذف' and is_owner(msg) then
			if matches[2] == 'bans' or matches[2] == 'بن' then
				if next(data[tostring(chat)]['banned']) == nil then
     if not lang then
					return "⚠️_No_ *banned* _users in this group_⚠️"
   else
					return "⚠️*هیچ کاربری از این گروه محروم نشده*⚠️"
              end
				end
     ...
     if not lang then
				return "⚠️_All_ *banned* _users has been unbanned_⚠️"
    else
				return "⚠️*تمام کاربران محروم شده از گروه از محرومیت خارج شدند*⚠️"
           end
			end
			if matches[2] == 'silentlist' or matches[2] == 'لیست سایلنت' then
				if next(data[tostring(chat)]['is_silent_users']) == nil then
        if not lang then
					return "⚠️_No_ *silent* _users in this group_⚠️"
   else
					return "⚠️*لیست کاربران سایلنت شده خالی است*⚠️"
             end
				end
    ...
    if not lang then
					...
					return "⚠️*هیچ کاربری از گروه های ربات محروم نشده*⚠️"
             end
				end
      ...
      if not lang then
				return "⚠️_All_ *globally banned* _users has been unbanned_⚠️"
   else
				return "⚠️*تمام کاربرانی که از گروه های ربات محروم بودند از محرومیت خارج شدند*⚠️"
          end
			end
     end
